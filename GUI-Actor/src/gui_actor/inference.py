...
def get_prediction_region_point(attn_scores, n_width, n_height, top_n=50, activation_threshold=0.15, return_all_regions=True, rect_center=False, min_region_size=2):
    """
    ...
    valid_regions = [(r, len(r)) for r in regions if len(r) >= min_region_size]
    if not valid_regions:
        valid_regions = [(r, len(r)) for r in regions]
    
    region_scores = []
    region_centers = []
    region_points = []
    
    for region, _ in valid_regions:
    ...
    combined_scores = []
    for i, region in enumerate([r for r, _ in valid_regions]):
        size_bonus = min(len(region) / 10.0, 1.0)
        combined = region_scores[i] * (1 + 0.3 * size_bonus)
...
def inference(conversation, model, tokenizer, data_processor, logits_processor=None, use_placeholder=False, topk=5):
    ...
    best_point, region_points, region_scores, region_points_all = get_prediction_region_point(attn_scores, n_width, n_height, top_n=50, activation_threshold=0.15, return_all_regions=True, rect_center=False)
